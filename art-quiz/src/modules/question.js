...
import { isMuted, saundValue, timeGame, timeOfGame } from './settings';
...
class Question {
  ...
  async timeGames() {
    try {
      ...
      const { category } = this;
      const { isArtist } = this;
      const { quizQuestions } = this;
      const { categoryText } = this;
      const { dynamicLine } = this;
  ...
  async getRaund() {
    try {
      ...
      if (this.isArtist) {
        ...
        arrRaund = data
          .filter((pic) => pic.category === this.category)
          .reverse()
          .slice(0, 10);
      }

      const { imageNum } = arrRaund[this.counter];
      const { author } = arrRaund[this.counter];
      const { year } = arrRaund[this.counter];
      const { name } = arrRaund[this.counter];
  ...
  async getRandomAutors() {
    try {
      ...
      for (let i = 0; i < 4 + counter; i++) {
        const randomAuthor = data[getRandomNumber(data.length)].author;
  ...
  async getRandomPictures() {
    try {
      ...
      for (let i = 0; i < 4 + counter; i++) {
        const randomPicture = data[getRandomNumber(data.length)].imageNum;
